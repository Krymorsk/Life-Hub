...
function renderSettings(){
  ...
  const name=user?.displayName || state.profile?.displayName || "Arish";
  const email=user?.email || "Email/password account";
  const photo=user?.photoURL || "";
  const initials=(name.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join("")||"A").toUpperCase();

  return `
    <div class="settings-page">
      <section class="settings-profile glass">
        <div class="settings-profile-avatar">
          ${photo
            ? `<img src="${esc(photo)}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span style="display:none">${esc(initials)}</span>`
            : `<span>${esc(initials)}</span>`}
        </div>
        <div class="settings-profile-copy">
          <span class="section-kicker">YOUR LIFE HUB</span>
          <h3>${esc(name)}</h3>
          <p>${esc(email)}</p>
          <div class="settings-chips">
            <span class="settings-chip positive-chip">${user?"Connected":"Offline"}</span>
            <span class="settings-chip">${user?.isAnonymous?"Guest":"Account"}</span>
          </div>
        </div>
        <button class="mini-action settings-profile-action" id="accountBtn">${user?.isAnonymous?"Secure account":"Account"}</button>
      </section>

      <section class="settings-group">
        <div class="settings-group-head">
          <span class="section-kicker">ACCOUNT</span>
          <small>Your identity and access</small>
        </div>
        <div class="settings-cards">
          <button class="settings-card glass" id="settingsResetPassword" type="button">
            <span class="settings-icon">⌁</span>
            <span class="settings-card-copy"><b>Password</b><small>Send a secure reset email to ${esc(email)}.</small></span>
            <span class="settings-chevron">→</span>
          </button>
          <button class="settings-card glass" id="signOutRow" type="button">
            <span class="settings-icon soft-red">↪</span>
            <span class="settings-card-copy"><b>Sign out</b><small>Return safely to the Life Hub login screen.</small></span>
            <span class="settings-chevron">→</span>
          </button>
        </div>
      </section>
      ...
      <section class="settings-group">
        <div class="settings-group-head">
          <span class="section-kicker">APP & DATA</span>
          <small>How Life Hub stores and protects your information</small>
        </div>
        <div class="settings-cards">
          <div class="settings-card glass">
            <span class="settings-icon">☁</span>
            <span class="settings-card-copy"><b>Cloud database</b><small>Cloud Firestore is the authoritative source.</small></span>
            <span class="status-pill positive-chip">Connected</span>
          </div>
          <div class="settings-card glass">
            <span class="settings-icon">◎</span>
            <span class="settings-card-copy"><b>Firebase identity</b><small>${user?`Authenticated · ${user.isAnonymous?"anonymous":"account"}`:"Not connected"}</small></span>
            <span class="status-pill ${user?"positive-chip":"negative-chip"}">${user?"Active":"Offline"}</span>
          </div>
          <div class="settings-card glass">
            <span class="settings-icon">◈</span>
            <span class="settings-card-copy"><b>Data model</b><small>Versioned records and relationships.</small></span>
            <span class="status-pill">V4</span>
          </div>
          <div class="settings-card glass" id="exportRow">
            <span class="settings-icon">↓</span>
            <span class="settings-card-copy"><b>Export data</b><small>Download a complete Firestore snapshot as JSON.</small></span>
            <span class="settings-chevron">→</span>
          </div>
        </div>
      </section>
      ...
      <section class="settings-group">
        <div class="settings-group-head">
          ...
          <small>Keep important things visible without making Life Hub noisy</small>
        </div>
        <div class="settings-cards">
          <div class="settings-card glass">
            <span class="settings-icon">◌</span>
            <span class="settings-card-copy"><b>Browser alerts</b><small>Works while Life Hub is active.</small></span>
            <button class="mini-action" id="notifySettingsBtn" type="button">${("Notification" in window && Notification.permission==="granted")?"Enabled":"Enable"}</button>
          </div>
        </div>
      </section>
      ...
      <section class="settings-group danger-group">
        <div class="settings-group-head">
          <span class="section-kicker danger-kicker">DANGER ZONE</span>
          <small>Destructive actions live here so they stay out of your way.</small>
        </div>
        <button class="settings-danger glass" id="resetRow" type="button">
          <div><b>Reset all Life Hub data</b><small>Delete this user's Firestore records. This cannot be undone.</small></div>
          <span>Reset →</span>
        </button>
      </section>
